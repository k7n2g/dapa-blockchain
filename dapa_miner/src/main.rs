...
#[derive(Parser, Serialize, Deserialize)]
#[clap(version = VERSION, about = "DAPA an Advanced Cryptocurrency built with BlockDAG, Homomorphic Encryption, Zero-Knowledge Proofs, and Smart Contracts.")]
...
async fn communication_task(daemon_address: String, job_sender: broadcast::Sender<ThreadNotification<'_>>, mut block_receiver: mpsc::Receiver<MinerWork<'_>>, address: Address, worker: String) {
    ...
    'main: loop {
        ...
        let client = match connect_async(format!("wss://{}/getwork/{}/{}", daemon_address, address.to_string(), worker)).await {
            Ok((client, response)) => {
...
async fn run_prompt(prompt: ShareablePrompt) -> Result<()> {
    ...
    let closure = |_: &_, _: _| async {
        ...
        Ok(
            format!(
                "{} | {} | {} | {} | {} | {} {} ",
                prompt.colorize_str(Color::Red, "DAPA Miner"),
                topoheight_str,
                blocks_found,
                blocks_rejected,
                hashrate,
