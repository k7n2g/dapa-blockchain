...
pub struct LogConfig {
    ...
    /// If disabled, the log file will be named dapa-miner.log instead of YYYY-MM-DD.dapa-miner.log
    #[clap(long)]
    #[serde(default)]
    ...
    /// Log filename
    /// 
    /// By default filename is dapa-miner.log.
    /// File will be stored in logs directory, this is only the filename, not the full path.
    /// Log file is rotated every day and has the format YYYY-MM-DD.dapa-miner.log.
    #[clap(default_value_t = String::from("dapa-miner.log"))]
...
#[derive(Parser, Serialize, Deserialize)]
#[clap(version = VERSION, about = "DAPA is a Cryptocurrency with BlockDAG, Homomorphic Encryption, Zero-Knowledge Proofs, and Smart Contracts.")]
...
fn start_thread(id: u16, mut job_receiver: broadcast::Receiver<ThreadNotification<'static>>, block_sender: mpsc::Sender<MinerWork<'static>>) -> Result<std::thread::JoinHandle<Result<(), Error>>, Error> {
...
async fn run_prompt(prompt: ShareablePrompt) -> Result<()> {
    ...
    let closure = |_: &_, _: _| async {
        ...
        Ok(
            format!(
                "{} | {} | {} | {} | {} | {} {} ",
                prompt.colorize_string(Color::Blue, "DAPA Miner"),
                topoheight_str,
                blocks_found,
                blocks_rejected,
                hashrate,
