...
pub fn build_environment<P: ContractProvider>(version: ContractVersion) -> EnvironmentBuilder<'static, ContractMetadata> {
    ...
    // See dapa_common::contract::opaque::storage::BTreeSeekBias
    ...
    if version >= ContractVersion::V1 {
        ...
        // returns the DAPA asset hash
        env.register_native_function(
            "get_dapa_asset",
            None,
            vec![],
            FunctionHandler::Sync(get_dapa_asset),
            1,
            Some(hash_type.clone())
        );
    }
...
fn get_dapa_asset(_: FnInstance, _: FnParams, _: &ModuleMetadata<'_>, _: &mut VMContext) -> FnReturnType<ContractMetadata> {
    Ok(SysCallResult::Return(Primitive::Opaque(OpaqueWrapper::new(DAPA_ASSET.clone())).into()))
}
...
async fn increase_gas_limit<'a, 'ty, 'r, P: ContractProvider>(_: FnInstance<'a>, params: FnParams, metadata: &ModuleMetadata<'_>, context: &mut VMContext<'ty, 'r>) -> FnReturnType<ContractMetadata> {
    ...
    // and gas limit will be paid to the miners
    {
        ...
        let balance = match state.global_caches.get(&metadata.metadata.contract_executor)
            .and_then(|cache| cache.balances.get(&DAPA_ASSET).map(Option::as_ref).flatten()) {
                Some((_, balance)) => *balance,
                None => {
                    let (_, balance) = provider.get_contract_balance_for_asset(&metadata.metadata.contract_executor, &DAPA_ASSET, state.topoheight).await?
    ...
    if !has_enough_balance_for_contract(provider, state, metadata.metadata.contract_executor.clone(), DAPA_ASSET, amount).await? {
        return Ok(SysCallResult::Return(Primitive::Boolean(false).into()));
    }

    record_balance_charge(provider, state, metadata.metadata.contract_executor.clone(), DAPA_ASSET, amount).await?;
