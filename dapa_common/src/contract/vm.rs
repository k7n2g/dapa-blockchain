...
use crate::{
    config::{TX_GAS_BURN_PERCENT, DAPA_ASSET, CONTRACT_MAX_PAYLOAD_SIZE, CONTRACT_PAYLOAD_FEE_PER_BYTE},
    contract::{
        ChainState,
        ContractCache,
        ContractLog,
    ...
    for (source, gas) in gas_sources.into_iter() {
        ...
        match source {
            ...
            Source::Account(account) => {
                ...
                let balance = state.get_receiver_balance(Cow::Owned(account), Cow::Owned(DAPA_ASSET)).await
                    .map_err(ContractError::State)?;
        ...
        match source {
            Source::Contract(contract) => {
                if gas_refund_left > 0 {
                    ...
                    let (state, balance) = cache.balances.get_mut(&DAPA_ASSET)
...
pub async fn handle_gas<'a, P: ContractProvider, E, B: BlockchainApplyState<'a, P, E>>(
    ...
    if refund_gas > 0 {
        ...
        if let ContractCaller::Transaction(_, tx) = caller {
            let balance = state.get_receiver_balance(Cow::Borrowed(tx.get_source()), Cow::Owned(DAPA_ASSET)).await
                .map_err(ContractError::State)?;
