...
async fn test_blockchain_apply_state_gas_tracking() {
    ...
    state.add_burned_coins(&DAPA_ASSET, 100).await.unwrap();
    assert_eq!(state.burned_coins.get(&DAPA_ASSET), Some(&100));
    
    state.add_burned_coins(&DAPA_ASSET, 50).await.unwrap();
    assert_eq!(state.burned_coins.get(&DAPA_ASSET), Some(&150));
}
...
async fn test_contract_balance_for_gas() {
    ...
    // Simulate fetching from storage
    {
        ...
        let (versioned_state, _) = cache.balances.get_mut(&DAPA_ASSET)
            .unwrap()
            .as_mut()
            .unwrap();
...
async fn test_refund_gas_sources_single_account() {
    ...
    // Initialize account with balance (encrypted)
    {
        ...
        state.accounts.insert(account.clone(), MockAccount {
            balances: [(DAPA_ASSET, balance_ct)].into_iter().collect(),
            nonce: 0,
        });
    }
    ...
    let balance_ct = &state.accounts.get(&account).unwrap().balances[&DAPA_ASSET];
