...
async fn schedule_execution<'a, 'ty, 'r, P: ContractProvider>(
    ...
    let source = if use_contract_balance {
        ...
        if !has_enough_balance_for_contract(provider, state, metadata.metadata.contract_executor.clone(), DAPA_ASSET, total_cost).await?{
            return Ok(SysCallResult::Return(Primitive::Null.into()));
        }
    ...
    if use_contract_balance {
        ...
        record_burned_asset(provider, state, metadata.metadata.contract_executor.clone(), DAPA_ASSET, burned_part).await?;
