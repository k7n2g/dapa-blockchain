...
use crate::{
    ...
    config::{BURN_PER_CONTRACT, COIN_VALUE, DAPA_ASSET},
...
fn create_tx_for(account: Account, destination: Address, amount: u64, extra_data: Option<DataElement>) -> Arc<Transaction> {
    ...
    let balance = state.balances[&DAPA_ASSET].balance;
    ...
    let new_balance = state.balances[&DAPA_ASSET].balance;
...
fn test_encrypt_decrypt_two_parties() {
    ...
    alice.balances.insert(DAPA_ASSET, Balance {
...
async fn test_tx_verify() {
    ...
    alice.set_balance(DAPA_ASSET, 100 * COIN_VALUE);
    bob.set_balance(DAPA_ASSET, 0);
    ...
    // Check Bob balance
    let balance = bob.keypair.decrypt_to_point(&state.accounts[&bob.keypair.get_public_key().compress()].balances[&DAPA_ASSET]);    
    assert_eq!(balance, Scalar::from(50u64) * (*G));

    // Check Alice balance
    let balance = alice.keypair.decrypt_to_point(&state.accounts[&alice.keypair.get_public_key().compress()].balances[&DAPA_ASSET]);
...
async fn test_tx_invoke_contract() {
    ...
    let tx = {
        ...
        let data = TransactionTypeBuilder::InvokeContract(InvokeContractBuilder {
            ...
            parameters: Vec::new(),
            deposits: [
                (DAPA_ASSET, ContractDepositBuilder {
                    amount: 50 * COIN_VALUE,
                    private: false
                })
            ].into_iter().collect(),
    ...
    let tx = {
        ...
        let data = TransactionTypeBuilder::DeployContract(DeployContractBuilder {
            ...
            invoke: Some(DeployContractInvokeBuilder {
                deposits: [(DAPA_ASSET, ContractDepositBuilder {
                    amount: deposit,
                    private: false
                })].into(),
                max_gas,
