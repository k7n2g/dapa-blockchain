...
use crate::{
    account::Nonce,
    time::Instant,
    config::{BURN_PER_CONTRACT, MAX_GAS_USAGE_PER_TX, DAPA_ASSET},
    contract::{
        vm::{
            self,
            ContractCaller,
...
impl Transaction {
    ...
    fn verify_commitment_assets(&self) -> bool {
        ...
        // DAPA_ASSET is always required for fees
        if !has_commitment_for_asset(&DAPA_ASSET) {
            return false;
        }
        ...
        C: ZKPCache<E>
    {
        ...
        histogram!("dapa_verify_batch_pre_ms").record(start.elapsed().as_millis() as f64);
        ...
        spawn_blocking_safe(move || {
            ...
            histogram!("dapa_verify_batch_collector_ms").record(start.elapsed().as_millis() as f64);
            ...
            if !prepared.is_empty() {
                ...
                histogram!("dapa_verify_batch_range_ms").record(start.elapsed().as_millis() as f64);
            } else {
        ...
        histogram!("dapa_verify_batch_ms").record(start.elapsed().as_millis() as f64);
