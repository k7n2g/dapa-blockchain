...
pub async fn verify_fee<P: AccountProvider>(
    ...
    let (fee_paid, refund) = if required_fees > tx.get_fee() {
        ...
        if required_fees > tx.get_fee_limit() {
            debug!("Invalid fees: {} required, {} provided", format_dapa(required_fees), format_dapa(tx.get_fee()));
...
async fn is_referencing_previous_output<S: Storage>(storage: &S, tx: &Transaction, reference: &Reference, topoheight: TopoHeight) -> Result<bool, BlockchainError> {
    ...
    debug!("Checking if TX is referencing previous output between topoheights {} and {} (reference: {})", min_topo, topoheight, reference_block_topo);
    if let Some((topo, version)) = storage.get_output_balance_in_range(tx.get_source(), &DAPA_ASSET, min_topo, topoheight).await? {
        debug!("Found output balance for DAPA at topoheight {}", topo);
