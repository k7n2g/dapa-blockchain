...
impl<S: Storage> Blockchain<S> {
    ...
    pub async fn reload_from_disk_with_storage(&self, storage: &mut S) -> Result<(), BlockchainError> {
        ...
        counter!("dapa_blockchain_reload_from_disk").increment(1);
    ...
    async fn create_genesis_block(&self, genesis_hex: Option<&str>) -> Result<(), BlockchainError> {
        ...
        let genesis_block = {
            ...
            // register DAPA asset
            debug!("Registering DAPA asset: {} at topoheight 0", DAPA_ASSET);
            let ticker = match self.network {
                Network::Mainnet => "DAPA".to_owned(),
                _ => "DAH".to_owned(),
            };
            ...
            storage.add_asset(
                ...
                VersionedAssetData::new(
                    AssetData::new(COIN_DECIMALS, "DAPA".to_owned(), ticker, MaxSupplyMode::Fixed(MAXIMUM_SUPPLY), AssetOwner::None),
                    None
                )
            ).await?;
    ...
    pub async fn prune_until_topoheight_for_storage(&self, topoheight: TopoHeight, storage: &mut S) -> Result<TopoHeight, BlockchainError> {
        ...
        if located_sync_topoheight > last_pruned_topoheight {
            ...
            counter!("dapa_blockchain_prune_until_topoheight").increment(1);
    ...
    // Get the current emitted supply of DAPA at current topoheight
        ...
        I: Iterator<Item = &'a Hash> + ExactSizeIterator + Clone,
    {
        ...
        histogram!("dapa_difficulty_time_µs").record(elapsed.as_micros() as f64);
    ...
    pub async fn add_tx_to_mempool_internal(
        ...
        let hash = {
            ...
            histogram!("dapa_mempool_tx_added_ms").record(start.elapsed().as_millis() as f64);
            counter!("dapa_txs_verified").increment(1u64);
    ...
    pub async fn get_block_header_template_for_storage(&self, storage: &S, address: PublicKey) -> Result<BlockHeader, BlockchainError> {
        ...
        histogram!("dapa_block_header_template_ms").record(start.elapsed().as_millis() as f64);
    ...
    pub async fn get_block_template_for_storage(&self, storage: &S, address: PublicKey) -> Result<BlockHeader, BlockchainError> {
        ...
        if !tx_selector.is_empty() {
            ...
            while let Some(TxSelectorEntry { size, hash, tx, fee_per_kb, fee_limit_per_kb }) = tx_selector.next() {
                ...
                if fee_per_kb < base_fee && fee_limit_per_kb < base_fee {
                    debug!("Skipping TX {} because it has a lower fee per kb ({}, limit {}) than required base fee ({})", hash, format_dapa(fee_per_kb), format_dapa(fee_limit_per_kb), format_dapa(base_fee));
                ...
                trace!("Selected {} (nonce: {}, fees: {}) for mining", hash, tx.get_nonce(), format_dapa(tx.get_fee()));
                // TODO no clone
        ...
        histogram!("dapa_block_header_template_txs_selection_ms").record(start.elapsed().as_millis() as f64);
        counter!("dapa_block_template").increment(1);
    ...
    pub async fn pre_verify_block(&self, block: &Block, block_hash: Option<Immutable<Hash>>) -> Result<PreVerifyBlock, BlockchainError> {
        ...
        if self.skip_pow_verification() {
            ...
            spawn_blocking(move || {
                ...
                histogram!("dapa_block_pow_ms").record(start.elapsed().as_millis() as f64);
    ...
    pub async fn add_new_block_with_storage(&self, holder: StorageHolder<'_, S>, block: Block, pre_verify: PreVerifyBlock, broadcast: BroadcastOption, mining: bool) -> Result<(), BlockchainError> {
        ...
        // If the TX is executed by another branch, we skip the verification because DAG will choose which branch will execute the TX
        {
            ...
            if !txs_grouped.is_empty() {
                ...
                // Record metrics
                counter!("dapa_txs_verified").increment(total_txs as u64);
                histogram!("dapa_txs_verification_ms").record(start.elapsed().as_millis() as f64);
            }
        }
        ...
        counter!("dapa_block_added").increment(1);
        ...
        // Save transactions & block
        {
            ...
            histogram!("dapa_block_store_ms").record(start.elapsed().as_millis() as f64);
        ...
        let mut dag_is_overwritten = base_topo_height == 0;
        {
            ...
            for (i, hash) in full_order.into_iter().enumerate() {
                ...
                let changes = chain_state.get_asset_changes_for(&DAPA_ASSET, true).await?;
            ...
            // Record metrics
            counter!("dapa_txs_executed").increment(total_txs_executed as u64);
            histogram!("dapa_txs_execution_ms").record(elapsed.as_millis() as f64);
            histogram!("dapa_dag_ordering_ms").record(start.elapsed().as_millis() as f64);
        }
        ...
        // trace!("final storage write lock acquired for {}", block_hash);
        {
            ...
            if chain_height_extended {
                ...
                gauge!("dapa_block_height").set(current_height as f64);
            }

            if chain_topoheight_extended {
                chain_cache.topoheight = current_topoheight;
                gauge!("dapa_block_topoheight").set(current_topoheight as f64);
            }

            gauge!("dapa_block_difficulty").set(difficulty.as_u64().unwrap_or(0) as f64);
        ...
        {
            counter!("dapa_orphaned_txs").increment(orphaned_transactions.len() as u64);
            ...
            histogram!("dapa_orphaned_txs_add_back_ms").record(start.elapsed().as_millis() as f64);
        }
        ...
        let mempool_deleted_txs = if chain_topoheight_extended {
            ...
            histogram!("dapa_mempool_clean_up_ms").record(start.elapsed().as_millis() as f64);
        ...
        // Record metrics
        histogram!("dapa_block_processing_ms").record(elapsed as f64);
        ...
        if let Some(rpc) = rpc_server.as_ref() {
            ...
            if broadcast.miners() {
                if let Some(getwork) = rpc.getwork_server() {
                    ...
                    spawn_task("notify-new-job", async move {
                        ...
                        histogram!("dapa_notify_new_job_ms").record(start.elapsed().as_millis() as f64);
                    });
                }
            }
            ...
            spawn_task("rpc-notify-events", async move {
                ...
                histogram!("dapa_new_block_notify_events_ms").record(start.elapsed().as_millis() as f64);
            });
        }
    ...
    pub async fn internal_get_block_reward(&self, past_supply: u64, is_side_block: bool, side_blocks_count: u64, version: BlockVersion) -> Result<u64, BlockchainError> {
        ...
        // PREMINE: If genesis block (past_supply == 0), return premine amount
    if past_supply == 0 {
        use crate::config::PREMINE_AMOUNT;
        return Ok(PREMINE_AMOUNT);
    }
    ...
    pub async fn rewind_chain_for_storage(&self, storage: &mut S, count: u64, stop_at_stable_height: bool) -> Result<(TopoHeight, Vec<(Hash, Immutable<Transaction>)>), BlockchainError> {
        ...
        counter!("dapa_rewind_chain").increment(1);
        histogram!("dapa_rewind_chain_count").record(count as f64);
        ...
        histogram!("dapa_rewind_chain_ms").record(start.elapsed().as_millis() as f64);
