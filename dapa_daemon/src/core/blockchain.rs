...
use dapa_common::{
    ...
    utils::{calculate_tx_fee, format_dapa},
...
impl<S: Storage> Blockchain<S> {
    ...
    async fn create_genesis_block(&self, genesis_hex: Option<&str>) -> Result<(), BlockchainError> {
        ...
        storage.add_asset(
            ...
            VersionedAssetData::new(
                AssetData::new(COIN_DECIMALS, "DAPA".to_owned(), ticker, Some(MAXIMUM_SUPPLY), None),
                None
            )
        ).await?;
    ...
    // Get the current burned supply of DAPA at current topoheight
