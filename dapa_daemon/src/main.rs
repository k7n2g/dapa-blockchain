...
use dapa_common::{
    async_handler,
    config::{init, VERSION, DAPA_ASSET},
    context::Context,
    crypto::{
        Address,Hashable
    },
...
pub struct LogConfig {
    ...
    /// If disabled, the log file will be named dapa-daemon.log instead of YYYY-MM-DD.dapa-daemon.log
    #[clap(long)]
    #[serde(default)]
    ...
    /// Log filename
    /// 
    /// By default filename is dapa-daemon.log.
    /// File will be stored in logs directory, this is only the filename, not the full path.
    /// Log file is rotated every day and has the format YYYY-MM-DD.dapa-daemon.log.
    #[clap(long, default_value_t = String::from("dapa-daemon.log"))]
    ...
    for topo in pruned_topoheight..=topoheight {
        ...
        let block_reward = if pruned_topoheight == 0 || topo - pruned_topoheight > STABLE_LIMIT {
            ...
            if block_reward != expected_block_reward {
                manager.error(format!("Block reward saved is incorrect for {} at topoheight {}, got {} while expecting {}", hash_at_topo, topo, format_dapa(block_reward), format_dapa(expected_block_reward)));
                return Ok(())
            }
            block_reward
        } else {
        ...
        for tx_hash in header.get_transactions() {
            if storage.is_tx_executed_in_block(tx_hash, &hash_at_topo).context("Error while checking if tx is executed in block")? {
                ...
                if let Some(burned) = transaction.get_burned_amount(&DAPA_ASSET) {
                    burned_sum += burned;
                }
            }
        }
        ...
        if burned_supply != expected_burned_supply {
            manager.error(format!("Error for block {} at topoheight {}, expected burned supply {} found {}", hash_at_topo, topo, format_dapa(expected_burned_supply), format_dapa(burned_supply)));
            return Ok(())
        }
    }
...
async fn status<S: Storage>(manager: &CommandManager, _: ArgumentManager) -> Result<(), CommandError> {
    ...
    manager.message(format!("Circulating Supply: {} DAPA", format_dapa(emitted_supply - burned_supply)));
    manager.message(format!("Current Block Reward: {} DAPA", format_dapa(get_block_reward(emitted_supply))));
