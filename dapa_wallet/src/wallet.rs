        ...
        for tx in transactions {
            match tx.get_entry() {
                ...
                EntryData::InvokeContract { contract, deposits, chunk_id, fee, max_gas, nonce } => {
                    ...
                    str_deposits.push(format!("Gas:{}", format_dapa(*max_gas)));
                    for (asset, amount) in deposits {
                        let data = storage.get_asset(&asset).await?;
                        str_deposits.push(format!("{}:{}", data.get_name(), format_coin(*amount, data.get_decimals())));
                    }

                    writeln!(w, "{},{},{},{},{},{},{},{},{}", datetime_from_timestamp(tx.get_timestamp())?, tx.get_topoheight(), tx.get_hash(), "InvokeContract", contract, str_deposits.join("|"), chunk_id, format_dapa(*fee), nonce).context("Error while writing csv line")?;
                },
                EntryData::DeployContract { fee, nonce } => {
                    writeln!(w, "{},{},{},{},-,-,-,{},{}", datetime_from_timestamp(tx.get_timestamp())?, tx.get_topoheight(), tx.get_hash(), "DeployContract", format_dapa(*fee), nonce).context("Error while writing csv line")?;
                },
            }
        }
