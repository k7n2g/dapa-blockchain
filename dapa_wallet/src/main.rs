...
use dapa_wallet::config::DEFAULT_DAEMON_ADDRESS;
    ...
    if let Some(manager) = command_manager {
        ...
        if let Ok(wallet) = context.get::<Arc<Wallet>>() {
            ...
            let balance = format!(
                ...
                prompt.colorize_string(Color::Green, &format_dapa(storage.get_plaintext_balance_for(&DAPA_ASSET).await.unwrap_or(0))),
            );
            ...
            return Ok(
                format!(
                    "{} | {} | {} | {} | {} {}{} ",
                    prompt.colorize_string(Color::Blue, "DAPA Wallet"),
                    addr_str,
                    topoheight_str,
                    balance,
                    status,
...
async fn untrack_asset(manager: &CommandManager, mut args: ArgumentManager) -> Result<(), CommandError> {
    ...
    if asset == DAPA_ASSET {
        manager.message("DAPA asset cannot be untracked");
...
async fn transfer(manager: &CommandManager, mut args: ArgumentManager) -> Result<(), CommandError> {
    ...
    manager.message(format!("Estimated TX fee is {} DAPA", format_dapa(estimated_fee)));
...
async fn transfer_all(manager: &CommandManager, mut args: ArgumentManager) -> Result<(), CommandError> {
    ...
    manager.message(format!("Sending {} of {} ({}) to {} (fees: {})", format_coin(amount, asset_data.get_decimals()), asset_data.get_name(), asset, address, format_dapa(estimated_fees)));
...
async fn deploy_contract(manager: &CommandManager, mut args: ArgumentManager) -> Result<(), CommandError> {
    ...
    let invoke = if module.get_chunk_id_of_hook(HOOK_CONSTRUCTOR_ID).is_some() {
        ...
        let max_gas = if args.has_argument("max_gas") {
            ...
            manager.message(format!("Maximum gas per transaction is set to {} DAPA, you cannot exceed this value.", format_dapa(MAX_GAS_USAGE_PER_TX)));
            read_asset_amount(prompt, wallet, &DAPA_ASSET).await?.0
        };
    ...
    manager.message(format!("Deploy a contract on the chain cost {} DAPA", format_dapa(BURN_PER_CONTRACT)));
...
async fn status(manager: &CommandManager, _: ArgumentManager) -> Result<(), CommandError> {
    ...
    if let Some(network_handler) = wallet.get_network_handler().lock().await.as_ref() {
        ...
        if is_online {
            ...
            manager.message(format!("Emitted supply: {}", format_dapa(info.emitted_supply)));
            manager.message(format!("Burned supply: {}", format_dapa(info.burned_supply)));
            manager.message(format!("Circulating supply: {}", format_dapa(info.circulating_supply)));
            manager.message("---------------------");
        }
    }
