    ...
    if let Some(manager) = command_manager {
        ...
        if let Ok(wallet) = context.get::<Arc<Wallet>>() {
            ...
            return Ok(
                format!(
                    "{} | {} | {} | {} | {} {}{} ",
                    prompt.colorize_str(Color::Blue, "DAPA Wallet"),
                    addr_str,
                    topoheight_str,
                    balance,
                    status,
...
async fn transfer_all(manager: &CommandManager, mut args: ArgumentManager) -> Result<(), CommandError> {
    ...
    manager.message(format!("Sending {} of {} to {} (fees: {})", format_coin(amount, decimals), asset, address.to_string(), format_dapa(estimated_fees)));
