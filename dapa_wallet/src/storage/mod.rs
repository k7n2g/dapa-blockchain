...
impl EncryptedStorage {
    pub fn new(inner: Storage, key: &[u8], salt: [u8; SALT_SIZE], network: Network) -> Result<Self> {
        ...
        // Force tracking of native dapa asset
        if !storage.is_asset_tracked_internal(&DAPA_ASSET)? {
            storage.track_asset_internal(&DAPA_ASSET)?;
        }
    ...
    pub fn get_filtered_transactions(
        ...
        for el in iterator.skip(skip.unwrap_or(0)) {
            ...
            match entry.get_mut_entry() {
                EntryData::Coinbase { .. } if accept_coinbase && (asset.map(|a| *a == DAPA_ASSET).unwrap_or(true)) => {},
