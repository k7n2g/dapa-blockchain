...
const PREFIX_DB_KEY: &'static str = "___dapa_db___";
...
mod tests {
    ...
    fn test_db_serialization() {
        ...
        assert_eq!(tree.get("test").unwrap().unwrap(), b"test".into());
        assert_eq!(tree.contains_key("dapa").unwrap(), true);
        assert_eq!(tree.get("dapa").unwrap().unwrap(), b"silex".into());
