...
impl TransactionEntry {
    ...
    pub async fn summary(&self, mainnet: bool, storage: &EncryptedStorage) -> Result<String> {
        let entry_str = match self.get_entry() {
            EntryData::Coinbase { reward } => format!("Coinbase {} DAPA", format_dapa(*reward)),
            EntryData::Burn { asset, amount, fee, nonce } => {
                let data = storage.get_asset(asset).await?;
                format!("Fee: {}, Nonce: {} Burn {} {} ({})", format_dapa(*fee), nonce, format_coin(*amount, data.get_decimals()), data.get_name(), asset)
            },
            EntryData::Incoming { from, transfers } => {
                let mut str = String::new();
                for transfer in transfers {
                    if *transfer.get_asset() == DAPA_ASSET {
                        str.push_str(&format!("Received {} DAPA from {}", format_dapa(transfer.get_amount()), from.as_address(mainnet)));
                    } else {
            ...
            EntryData::Outgoing { transfers, fee, nonce } => {
                let mut str = format!("Fee: {}, Nonce: {} ", format_dapa(*fee), nonce);
                for transfer in transfers {
                    if *transfer.get_asset() == DAPA_ASSET {
                        str.push_str(&format!("Sent {} DAPA to {}", format_dapa(transfer.get_amount()), transfer.get_destination().as_address(mainnet)));
                    } else {
                        let data = storage.get_asset(transfer.get_asset()).await?;
            ...
            EntryData::InvokeContract { contract, deposits, received, entry_id: chunk_id, fee, max_gas, nonce } => {
                let mut str = format!("Fee: {}, Nonce: {} ", format_dapa(*fee), nonce);
                str.push_str(&format!("Invoke contract {} with chunk id {} (max gas: {})", contract, chunk_id, format_dapa(*max_gas)));
                for (asset, amount) in deposits {
                    let data = storage.get_asset(&asset).await?;
            ...
            EntryData::DeployContract { fee, nonce, invoke } => {
                let invoke = match invoke {
                    Some(invoke) => {
                        ...
                        format!(" with invoke (max gas: {}, deposits: [{}])", format_dapa(invoke.max_gas), deposits)
                    },
                    None => String::new()
                };

                format!("Fee: {}, Nonce: {} Deploy contract{}", format_dapa(*fee), nonce, invoke)
            },
            EntryData::IncomingContract { transfers } => {
