...
| Address prefix | `dap:` on mainnet · `dat:` for testnet/devnet | When Available
| Transaction fee | 0.0001 DAPA per KB |
| Account creation fee | 0.001 DAPA |
| Per-transfer fee | 0.00005 DAPA |
| Decimals | 8 |
| Maximum supply | 800.000.000 million DAPA |
...
The daemon will automatically connect to the mainnet seed nodes and begin syncing.
...
./dapa_daemon \
  ...
  --getwork-bind-address 127.0.0.1:20101
```
...
- Binding GetWork to `127.0.0.1` prevents external miners from using your node.
...
For security, a user account should only broadcast TXs on the same chain/tip to prevent orphaned transactions. An orphaned TX can occur when two different TXs from the same owner with the same nonce are sent to two different branches.
...
Integrated addresses are base addresses with custom data embedded. For example, you can integrate a unique identifier that will be included in the future transaction made using it — helpful for linking a transaction to an account or order on the service side.

Maximum data allowed: **1 KB** (same as a transfer payload).
...
Wallets connected to a pruned node cannot retrieve transaction history before the pruned topoheight, but current balances remain accurate.

### Fast Sync

Fast sync allows you to join the network quickly by requesting a peer's chain state at a stable point (all account nonces, assets, balances, and top blocks). Use only with a **trusted peer**.

### Boost Sync

Requests the full chain in parallel rather than block-by-block, syncing much faster while preserving the full ledger. Enable with `--allow-boost-sync-mode`.

## Storage

All data is saved in plaintext using the **Sled** database engine.
...
The wallet tracks all your transactions and assets.

When creating a new wallet, a random **master key** is generated and encrypted with a hashed password. This master key encrypts/decrypts the entire wallet storage and can be re-encrypted to change your password without re-encrypting all data.

Password hashing algorithm: **Argon2id** with 15 MB memory and 16 iterations.

### Wallet Storage

The wallet uses a fully-encrypted storage system:
...
XSWD (DAPA Secure WebSocket DApp) Protocol runs on port **`44325`** at path **`/xswd`**.

It provides a secure channel from a desktop/CLI wallet to any dApp (browser or software). Instead of username/password authentication, XSWD requires the user to manually accept each dApp connection and can set per-method permissions.
...
Use the `node.` prefix to proxy requests to the daemon (no user approval needed as this is public chain data). Use the `wallet.` prefix for wallet methods (subject to user-configured permissions).
