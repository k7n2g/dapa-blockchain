...
DAPA Crypto Payments A Blockchain with **BlockDAG**, **Privacy**, **Speed**, **Scalability** and **Smart Contracts**.

## Features

The main features of DAPA are the following:
- **BlockDAG**: enabled to improve the scalability and the security of the network by virtual elimination of orphaned blocks.
- **Egalitarian PoW**: Unique PoW algorithm built to allow any CPU or GPU to mine DAPA easily using [xelis-hash](https://github.com/k7n2g/xelis-hash).
...
- **Privacy**: Homomorphic Encryption allows us to have encrypted balances and encrypted transfered amounts.
- **Smart Contracts**: allows us to create unstoppable applications by deploying programs on a decentralized network, executed in our sandboxed environment [xelis-vm](https://github.com/xelis-project/xelis-vm).
...
- Testnet: Running
- Devnet: this network is used for local development purpose. It has no peers
...
It's expected to be cross-platform and works on Linux, Windows, MacOS platforms.
...
DAPA is decentralized as the original bitcoin author envisioned crypto platforms to be, it is not funded by any company or organization, and Banks and Financial institutions do not control its value.
To help its development, and provide better support of DAPA, we set a dev fee percentage starting at 10% on block reward.

Current dev fee curve is as following:

- 10% from block 0 to 14,900.
- 5% from 14.901 until the project being developed is stable on major facets of the ecosystem in order to reduce it.
...
- Up to `8` decimals
- Maximum supply: `800.000.000` million
...
All hashes are calculated using the `Blake3` hashing algorithm except the Proof-Of-Work hash, which use [xelis-hash](https://github.com/k7n2g/xelis-hash).
