...
To build a specific binary from workspace (parent folder) directly, use the option `--bin` with ``, `dapa_miner` or `dapa_wallet` as value.
...
`docker build -t dapa-daemon:master --build-arg app= .`
...
- Address prefix is `dap` on mainnet andxel` for testnet/devnet
...
- Default RPC Server port is `8080`
