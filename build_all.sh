...
binaries=("dapa_daemon" "dapa_miner" "dapa_wallet")
...
export DAPA_COMMIT_HASH="$commit_hash"
...
for target in "${targets[@]}"; do
    ...
    if [[ "$target" == *"windows"* ]]; then
        ...
        docker run --rm -it -e DAPA_COMMIT_HASH="$commit_hash" -v "$PWD:/work" -w /work "$IMAGE_NAME" cargo xwin build --target "$target" --profile release-with-lto 
    else
        # ---- Non-Windows builds via cross ----
        DAPA_COMMIT_HASH="$commit_hash" cross build --target "$target" --profile release-with-lto
    fi
